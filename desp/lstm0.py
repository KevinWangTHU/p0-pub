...
class LSTM:
    ...
    def forward(self, inputs, masks):
        ...
        def step(x_t, xm_t, pre_c, pre_h, W, U, b):
            # (n_b, n_i) . (n_i, n_h * 4) + (n_b, n_h) . (n_h, n_h * 4) + (n_h * 4) => (n_b, n_h * 4)
        ...
        [cells, hiddens], update = theano.scan(fn=step,
...
def lstm_init(n_input, n_hidden):
    """
    init lstm params
    """
    return {'w': init_matrix_u((n_input, n_hidden * 4), 'w'),
            'u': init_matrix_u((n_hidden, n_hidden * 4), 'u'),
            'b': init_matrix_u((n_hidden * 4, ), 'b'),
            'h_0': init_matrix_u((n_hidden, ), 'h_0')}


def lstm_forward(params, inputs, masks):
    """
    @param param:  lstm
    @param inputs: [T((len, n_batch, n_input))]
    @param masks:  [T((len, n_batch))], 01 matrix
    ...
    [cells, hiddens], update = theano.scan(fn=step,
                                           sequences=[inputs, masks],
                                           outputs_info=[T.zeros_like(h_0s), h_0s],
                                           non_sequences=[params['w'], params['u'], params['b']])
