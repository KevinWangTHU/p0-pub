...
import dataproc
from util import *
# from trainer import log_setup, log_info
...
gflags.DEFINE_integer('n_embed', 10, 'Dimension of word embedding')
gflags.DEFINE_integer('n_hidden', 10, 'Dimension of hidden layer')
...
def compile_functions(model):
    ...
    params, loss, grad, rng_updates = \
        model.train([X_data, X_mask, X_pos, X_mask_d],
                    [Y_loss, Y_pre])
    func_params = [X_data, X_mask, X_pos, X_mask_d, Y_loss, Y_pre]
    lr, grad_shared, opt_updates = optimizer.optimize(params, {}, flags)
    # get_s = theano.function(func_params, s, on_unused_input='warn')
    # get_l = theano.function(func_params, l, on_unused_input='warn')
    # get_h = theano.function(func_params, h, on_unused_input='warn')
...
def load_data(flags):
    """
    @return:
    s_sentences, lasthid_enc, h_encs, upd_enc
    ...
    p_mask ~ [n_sent, n_doc_batch]
    Y_loss ~ [n_sent, n_doc_batch]
    Y_pre ~ [n_sent, n_doc_batch]
    ----
    X, Y
    """
    # _, (_, _, _, p_mask, Y_loss, Y_pre) = dataproc.load_data(flags)[0][2]
    # n_sent = p_mask.shape[0]
    # n_doc_batch = p_mask.shape[1]
    # n_hid = 5
    # n_layer = 2
    # s = np.zeros((n_sent, n_doc_batch, n_hid), dtype=np.float32)
    # l = np.ones((n_layer, n_doc_batch, n_hid), dtype=np.float32)
    # h = np.ones((n_sent, n_doc_batch, n_hid), dtype=np.float32)
    # return l, p_mask, s, h, Y_loss, Y_pre
    _, data = dataproc.load_data(flags)[0][2]
    return data


def train(flags):
    # compile
    model = models.RNNPicker()
    dropout_switch = model.dropout.switch
    # get_s, get_l, get_h = compile_functions(model)
    get_loss, update_params =compile_functions(model)
    log_info({"type": "function ready"})
